...
namespace Spaceports.Incidents
{
    public class IncidentWorker_VisitorShuttleArrival : IncidentWorker_NeutralGroup
    {
        ...
        {
            if (base.FactionCanBeGroupSource(f, parms, desperate) && !f.Hidden && !f.HostileTo(Faction.OfPlayer) &&
                f.def.pawnGroupMakers != null &&
                f.def.pawnGroupMakers.Any((PawnGroupMaker x) => x.kindDef == PawnGroupKindDef) &&
                f.def.techLevel.ToString() != "Neolithic")
            {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            LordJob lordJob = new LordJobs.LordJob_ShuttleVisitColony(parms.faction,
                Utils.GetBestChillspot(map, pad, 1), shuttle: shuttle.shipThing);
            ...
            if (LoadedModManager.GetMod<SpaceportsMod>().GetSettings<SpaceportsSettings>().visitorNotifications)
            {
                ...
                if (pawns.Count == 1)
                {
                    TaggedString taggedString2 = ((leader != null)
                        ? ("\n\n" + "Spaceports_SingleVisitorLeader".Translate(pawns[0].Named("PAWN"))
                            .AdjustedFor(pawns[0]))
                        : ((TaggedString)""));
                    notificationText = "Spaceports_SingleVisitorLanding".Translate(pawns[0].story.Title,
                            parms.faction.NameColored, pawns[0].Name.ToStringFull, taggedString2,
                            pawns[0].Named("PAWN"))
                        .AdjustedFor(pawns[0]);
                }
                else
                {
                    TaggedString taggedString4 = ((leader != null)
                        ? ("\n\n" + "Spaceports_GroupVisitorsLeader".Translate(leader.LabelShort, leader))
                        : TaggedString.Empty);
                    notificationText =
                        "Spaceports_GroupVisitorsLanding".Translate(parms.faction.NameColored, taggedString4);
                }
        ...
        private bool TryConvertOnePawnToSmallTrader(List<Pawn> pawns, Faction faction, Map map)
        {
            ...
            TraderKindDef traderKindDef =
                faction.def.visitorTraderKinds.RandomElementByWeight((TraderKindDef traderDef) =>
                    traderDef.CalculatedCommonality);
        ...
        protected override bool TryResolveParmsGeneral(IncidentParms parms)
        {
            ...
            {
                if (!parms.spawnCenter.IsValid &&
                    !CellFinder.TryFindRandomEdgeCellWith((cell) => true, map, CellFinder.EdgeRoadChance_Ignore, out parms.spawnCenter))
                {
                    return false;
                }
            }
