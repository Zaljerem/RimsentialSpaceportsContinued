...
namespace Spaceports.Incidents
{
        ...
        private static readonly SimpleCurve PointsCurve = new SimpleCurve
    {
        ...
        {
            if (base.FactionCanBeGroupSource(f, parms, desperate) && !f.Hidden && !f.HostileTo(Faction.OfPlayer) && f.def.pawnGroupMakers != null && f.def.pawnGroupMakers.Any((PawnGroupMaker x) => x.kindDef == PawnGroupKindDef) && f.def.techLevel.ToString() != "Neolithic")
            {
