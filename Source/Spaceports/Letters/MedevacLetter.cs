...
namespace Spaceports.Letters
{
    public class MedevacTracker : Utils.Tracker
    {
        ...
        {
            if (hurtPawn == null || shuttle == null)
            {
                Log.Warning("[Spaceports] MedevacTracker: hurtPawn or shuttle was null in Check()");
                return true; // Consider this tracker invalid and remove it
            }

            if (!hurtPawn.Downed && hurtPawn.Spawned)
            {
                List<Pawn> list = new List<Pawn> { hurtPawn };
                ...
                if (Rand.RangeInclusive(1, 100) <= 50)
                {
                    ...
                    var qi = new QueuedIncident(
                        new FiringIncident(SpaceportsDefOf.Spaceports_MedevacReward, null, incidentParms),
