    ...
    public class Utils
    {
        ...
        public static TransportShip GenerateInboundShuttle(List<Pawn> pawns, IntVec3 padCell, Map map, List<Thing> items = null, TransportShipDef forcedType = null, bool canLeave = true, bool dropAndGo = false)
        {
            ...
            if (pawns != null)
            {
                ...
                if (IsMapInSpace(map))
                {
                    if (map.Biome == outerSpaceBiome)
                    {
                        SuitUpPawnsSOS2(pawns);
                    }
                    else if (map.Biome == odysseySpaceBiome)
                    {
                        SuitUpPawnsOdyssey(pawns);
                    }
        ...
        public static bool IsMapInSpace(Map map) //Method to check if a map is a SOS2/Odyssey space map w/o assembly referencing
        {
            // Early exit if neither mod is loaded
            if (!Verse.ModLister.HasActiveModWithName("Save Our Ship 2") && !ModLister.OdysseyInstalled)
            {
                return false;
            }

            // Use safe lookups that won't throw if defs are missing
            BiomeDef outerSpaceBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("OuterSpaceBiome");
            BiomeDef odysseySpaceBiome = DefDatabase<BiomeDef>.GetNamedSilentFail("Space");

            return map.Biome == outerSpaceBiome || map.Biome == odysseySpaceBiome;
        }       

        public static void SuitUpPawnsSOS2(List<Pawn> pawns)//Method to equip a list of pawns with SOS2 suit + helmet w/o assembly referencing
        { 
        ...
        public static void SuitUpPawnsOdyssey(List<Pawn> pawns)//Method to equip a list of pawns with vacsuit + vacsuit helmet w/o assembly referencing
        {
